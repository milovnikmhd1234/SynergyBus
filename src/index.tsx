            ...
            >
              ...
              <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-500 uppercase mb-1">Zastávky (oddělené čárkou)</label>
                    <textarea 
                      value={tempConfig.stops.join(', ')}
                      onChange={e => setTempConfig({...tempConfig, stops: e.target.value.split(',').map(s => s.trim()).filter(Boolean)})}
                      className="w-full bg-[#222] border border-[#333] rounded-lg p-3 text-gray-300 text-xs outline-none focus:border-emerald-500 h-24"
                    />
                  </div>
                </div>
